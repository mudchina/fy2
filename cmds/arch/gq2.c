...
object *inv, weapon;
        ...
        {
        if( objectp(weapon = room->query_temp("weapon")) )
        {
        ...
        attack_points = COMBAT_D->skill_power(room, skill_type, SKILL_USAGE_ATTACK);
        parry_points = COMBAT_D->skill_power(room, parry_type, SKILL_USAGE_DEFENSE);
        dodge_points = COMBAT_D->skill_power(room, "dodge", SKILL_USAGE_DEFENSE);
        return sprintf("（%d，%d）",
                attack_points + 1,
